        ...
        <Card key={index} className="group relative overflow-hidden border-0 shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-300 cursor-pointer" hover>
          ...
          <Card.Body className="relative p-6">
            <div className="flex items-start justify-between">
              ...
              <div className={`p-4 rounded-2xl bg-gradient-to-br from-${stat.color}-500 to-${stat.color}-600 shadow-lg group-hover:scale-110 transition-transform duration-300`}>
                <stat.icon className="w-6 h-6 text-white" />
              </div>
            </div>
            
            {/* Decorative element */}
            <div className={`absolute bottom-0 right-0 w-16 h-16 bg-${stat.color}-200/20 rounded-full -translate-y-8 translate-x-8`}></div>
          </Card.Body>
        </Card>
      ))}
    </div>
