...
const DeviceList = () => {
  ...
  const fetchRealTimeData = async () => {
    try {
      const response = await devicesAPI.getAllRealTimeData();
  ...
  const handleAutoSync = async (device) => {
    try {
      ...
      if (!connectionTest.data.success) {
        setMessage({ type: 'warning', text: `Device ${device.name} is not connected` });
        return;
      }

      // Sync users
      const userSync = await devicesAPI.syncUsers(device.id);
      console.log('👥 Users synced:', userSync.data);

      // Sync attendance data
      const attendanceSync = await devicesAPI.syncAttendance(device.id);
      console.log('📅 Attendance synced:', attendanceSync.data);

      setMessage({ 
        type: 'success', 
        text: `Device ${device.name} synced successfully! Users: ${userSync.data.users_synced}, Records: ${attendanceSync.data.records_synced}` 
      });
      ...
      setMessage({ type: 'error', text: 'Auto sync failed' });
    } finally {
  ...
  const handleSyncAll = async () => {
    try {
      ...
      const response = await devicesAPI.syncAllDevices();
      console.log('✅ All devices synced:', response.data);
      
      setMessage({ 
        type: 'success', 
        text: `All devices synced successfully! Total records: ${response.data.total_records_synced}` 
      });
      
      fetchDevices(); // Refresh device list
      fetchRealTimeData(); // Refresh real-time data
    } catch (error) {
      console.error('❌ Sync all error:', error);
      setMessage({ type: 'error', text: 'Failed to sync all devices' });
    } finally {
      setSyncLoading(false);
    }
  };
  ...
  const getSystemStats = () => {
    ...
    return {
      ...
      totalUsers: realTimeData.total_users_system || 0,
      totalMappedUsers: realTimeData.total_mapped_users || 0,
      connectedDevices: devices.filter(d => d.is_enabled && d.status === 'active').length
    };
  };
