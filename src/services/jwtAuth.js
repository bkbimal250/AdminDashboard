...
class JWTAuthService {
  ...
  async login(credentials) {
    try {
      ...
      // Try to fetch full profile for richer personal info
      let user = this.decodeToken(access);
      try {
        const profileRes = await this.api.get('/users/profile/');
        if (profileRes?.data) {
          user = { ...user, ...profileRes.data };
        }
      } catch (_) {
        // fallback to decoded token user
      }
  ...
  async updateProfile(profileData) {
    try {
      const response = await this.api.put('/users/update_profile/', profileData);
      return {
        success: true,
        data: response.data
      };
